...
function ReplaceFontArr(array &$mapFontnameArr, string &$subsetASSContent, array &$subsetFontASSContent, bool $subsetFontOnly = false) {
	if (count($subsetFontASSContent) > 0) {
		...
		$comments = "";
		$randomToOriginal = [];
		foreach ($mapFontnameArr as $original => $random) {
			if (!isset($randomToOriginal[$random])) {
				$randomToOriginal[$random] = $original;
			}
		}
		foreach ($randomToOriginal as $random => $original) {
			$comments .= "; Font Subset: {$random} - {$original}\n";
		}
		$comments .= "; Script generated by FontServer (" . time() . ")\n";
		...
		$count = 0;
		$subsetASSContent = preg_replace('/(\[Script Info\][ \t]*\r?\n)/iu', '${1}' . $comments, $subsetASSContent, 1, $count);
		if ($count === 0) {
			$subsetASSContent .= "\n[Script Info]\n" . $comments;
		}
